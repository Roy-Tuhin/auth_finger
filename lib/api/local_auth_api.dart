import 'package:flutter/cupertino.dart';
import 'package:flutter/services.dart';
import 'package:local_auth/auth_strings.dart';
import 'package:local_auth/local_auth.dart';

class LocalAuthApi {
  static final _auth = LocalAuthentication(); 

  static Future<bool> hasBiometrics() async {
    try {
      return await _auth.canCheckBiometrics;
    } on PlatformException catch (e) {
      return false;
    }
  }

  static Future<bool> authenticateFinger() async { //!###################### only finger 
    final canCheck = await _auth.canCheckBiometrics; 

    if (canCheck) {
      List<BiometricType> availableBio = await _auth.getAvailableBiometrics(); 
      bool face = availableBio.contains(BiometricType.face); 
      bool fingerprint = availableBio.contains(BiometricType.fingerprint);

    //  fingerprint? 
     try{ 
       return  await _auth.authenticateWithBiometrics( 
        androidAuthStrings: AndroidAuthMessages( 
          signInTitle: 'Finger Required', 
        ),
        localizedReason: 'Scan your Finger for authentication', 
        useErrorDialogs: false, 
        stickyAuth: false, 
      );

     }on PlatformException catch (e) { 
      return false; 
    } 
      

     
    }

    // final isAvailable = await hasBiometrics();
    // if (!isAvailable) return false;

    try {
      return await _auth.authenticateWithBiometrics(
        androidAuthStrings: AndroidAuthMessages(
          signInTitle: 'Face ID Required',
        ),
        localizedReason: 'Scan Face to Authenticate',
        useErrorDialogs: false,
        stickyAuth: false,
      );
    } on PlatformException catch (e) {
      return false;
    }
  }

  //!###################################################################
   static Future<bool> authenticateFace() async { //!###################### only  F A C E 
    final canCheck = await _auth.canCheckBiometrics;
    

    if (canCheck) {
        List<BiometricType> availableBio = await _auth.getAvailableBiometrics();
      bool face = availableBio.contains(BiometricType.face);
      bool fingerprint = availableBio.contains(BiometricType.fingerprint);

    //  face? 
     try{
       return  await _auth.authenticateWithBiometrics(
        androidAuthStrings: AndroidAuthMessages(
          signInTitle: 'Face Required',
        ),
        localizedReason: 'Scan your Face for authentication',
        useErrorDialogs: false,
        stickyAuth: false,
      );

     }on PlatformException catch (e) {
      return false;
    }
     
    }

    
  }
}
